...
// V109: Electron 네이티브 confirm을 사용하지 않고 앱 내부 확인창을 사용합니다.
// 네이티브 confirm은 닫힌 뒤 현재 창이 클릭되지 않는 Electron 포커스 오류를 만들 수 있습니다.
...
function appConfirm(message, options={}){
  return new Promise(resolve=>{
    document.getElementById("seorinConfirmLayer")?.remove();
    const layer=document.createElement("div");
    layer.id="seorinConfirmLayer";
    layer.style.cssText="position:fixed;inset:0;z-index:2147483647;background:rgba(15,23,42,.38);display:flex;align-items:center;justify-content:center;padding:20px;pointer-events:auto";
    const dialog=document.createElement("div");
    dialog.setAttribute("role","dialog");
    dialog.setAttribute("aria-modal","true");
    dialog.style.cssText="width:min(430px,calc(100vw - 40px));background:#fff;border-radius:14px;box-shadow:0 24px 80px rgba(15,23,42,.28);padding:22px;font-family:inherit";
    const title=document.createElement("div");
    title.textContent=options.title||"확인";
    title.style.cssText="font-size:18px;font-weight:800;margin-bottom:12px;color:#0f172a";
    const body=document.createElement("div");
    body.textContent=String(message||"");
    body.style.cssText="white-space:pre-wrap;line-height:1.55;color:#334155;margin-bottom:20px";
    const actions=document.createElement("div");
    actions.style.cssText="display:flex;justify-content:flex-end;gap:8px";
    const cancel=document.createElement("button");
    cancel.type="button"; cancel.textContent=options.cancelText||"취소";
    cancel.style.cssText="border:1px solid #cbd5e1;background:#fff;border-radius:8px;padding:9px 16px;font-weight:700;cursor:pointer";
    const ok=document.createElement("button");
    ok.type="button"; ok.textContent=options.okText||"확인";
    ok.style.cssText=`border:0;background:${options.danger?"#dc2626":"#1677ff"};color:#fff;border-radius:8px;padding:9px 16px;font-weight:800;cursor:pointer`;
    actions.append(cancel,ok); dialog.append(title,body,actions); layer.append(dialog); document.body.append(layer);
    let done=false;
    const finish=value=>{
      ...
      requestAnimationFrame(()=>{restorePortalInteraction(); document.body.focus?.();});
...
// V117: Electron에서 지원하지 않는 prompt() 대신 앱 내부 입력창을 사용합니다.
function appPrompt(message, options={}){
  return new Promise(resolve=>{
    document.getElementById("seorinPromptLayer")?.remove();
    const layer=document.createElement("div");
    layer.id="seorinPromptLayer";
    layer.style.cssText="position:fixed;inset:0;z-index:2147483647;background:rgba(15,23,42,.38);display:flex;align-items:center;justify-content:center;padding:20px;pointer-events:auto";
    const dialog=document.createElement("div");
    dialog.setAttribute("role","dialog");
    dialog.setAttribute("aria-modal","true");
    dialog.style.cssText="width:min(460px,calc(100vw - 40px));background:#fff;border-radius:14px;box-shadow:0 24px 80px rgba(15,23,42,.28);padding:22px;font-family:inherit";
    const title=document.createElement("div");
    title.textContent=options.title||"입력";
    title.style.cssText="font-size:18px;font-weight:800;margin-bottom:10px;color:#0f172a";
    const body=document.createElement("div");
    body.textContent=String(message||"");
    body.style.cssText="white-space:pre-wrap;line-height:1.55;color:#334155;margin-bottom:12px";
    const input=options.multiline?document.createElement("textarea"):document.createElement("input");
    if(!options.multiline) input.type=options.type||"text";
    input.value=options.defaultValue??"";
    if(options.placeholder) input.placeholder=options.placeholder;
    input.style.cssText="width:100%;box-sizing:border-box;border:1px solid #cbd5e1;border-radius:9px;padding:11px 12px;font:inherit;outline:none;min-height:"+(options.multiline?"92px":"42px")+";resize:vertical";
    const actions=document.createElement("div");
    ...
    const ok=document.createElement("button");
    ok.type="button";ok.textContent=options.okText||"확인";
    ok.style.cssText="border:0;background:#1677ff;color:#fff;border-radius:8px;padding:9px 16px;font-weight:800;cursor:pointer";
    actions.append(cancel,ok);dialog.append(title,body,input,actions);layer.append(dialog);document.body.append(layer);
    let done=false;
    const finish=value=>{
      if(done)return;done=true;
      document.removeEventListener("keydown",onKey,true);
      layer.remove();restorePortalInteraction();
      requestAnimationFrame(restorePortalInteraction);setTimeout(restorePortalInteraction,30);
      resolve(value);
    };
    const onKey=e=>{
      if(e.key==="Escape"){e.preventDefault();finish(null);}
      if(e.key==="Enter"&&!options.multiline&&!e.shiftKey){e.preventDefault();finish(input.value);}
    };
    document.addEventListener("keydown",onKey,true);
    cancel.addEventListener("click",()=>finish(null));
    ok.addEventListener("click",()=>finish(input.value));
    layer.addEventListener("mousedown",e=>{if(e.target===layer)finish(null);});
    setTimeout(()=>{input.focus();if(options.selectAll!==false)input.select?.();},0);
  });
}
...
// 저장·삭제 버튼은 작업이 끝나면 항상 포커스를 해제합니다.
document.addEventListener("click",e=>{
  const button=e.target?.closest?.("button");
  if(button)setTimeout(()=>{button.blur?.();restorePortalInteraction();},0);
},true);
...
function fastRemoveFromState(key,id,renderFn){
  ...
  try{renderFn&&renderFn();}catch(e){console.warn('빠른 화면 갱신 실패',e);}
  requestAnimationFrame(()=>{const el=document.querySelector('input:not([type="hidden"]):not([disabled]), textarea:not([disabled]), select:not([disabled])');if(el&&document.activeElement===document.body)el.focus({preventScroll:true});});
}
function backgroundRefresh(fn){setTimeout(()=>Promise.resolve().then(fn).catch(e=>console.warn('백그라운드 갱신 실패',e)),250);}

function isRecordAdmin(){
  if(state.profile?.is_super_admin)return true;
  const perms=state.permissions||{};
  return ["card_manage","purchase_approve","purchase_final_approve","calendar_manage","meetings_manage","vehicles_manage","notices_manage","inventory_manage"].some(k=>!!perms[k]);
}
...
function dinnerAuthUserId(p){
  return p?.auth_user_id||p?.user_id||p?.profile_id||((typeof p?.id==="string"&&/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(p.id))?p.id:null);
}
function getDinnerEmployeeRows(){
  ...
  source.forEach(x=>{
    if(!x?.name)return;
    const key=normalizeDinnerName(x.name);
    const prev=byName.get(key);
    if(!prev||(!dinnerAuthUserId(prev)&&dinnerAuthUserId(x)))byName.set(key,x);
  });
  const rows=[];
  Object.entries(DINNER_FIXED_TEAMS).forEach(([team,names])=>{
    names.forEach(name=>{
      const p=byName.get(normalizeDinnerName(name))||{};
      const authId=dinnerAuthUserId(p);
      rows.push({...p,id:authId||`pending:${team}:${name}`,auth_user_id:authId,name,department:"물류본부",team,position:p.position||"",selectable:!!authId});
    });
  });
  source.forEach(p=>{
    const authId=dinnerAuthUserId(p);
    if(!authId||!p?.name||p.is_active===false||rows.some(r=>String(r.auth_user_id||r.id)===String(authId)))return;
    let team=String(p.team||p.department||"").trim();
    if(team==="B2C")team="B2C팀";
    if(team==="물류부")team="물류팀";
    if(!Object.prototype.hasOwnProperty.call(DINNER_FIXED_TEAMS,team))return;
    rows.push({...p,id:authId,auth_user_id:authId,department:"물류본부",team,position:p.position||"",selectable:true});
  });
...
function clearDinnerRoomForm(){
  ...
  state.selectedDinnerEmployeeIds=[];state.dinnerSelectedDepartment="";renderDinnerEmployeePicker();renderDinnerSelectedEmployees();
}
function renderDinnerEmployeePicker(){
  ...
  box.innerHTML=members.length
    ?`<div class="dinner-employee-group-list">${members.map(e=>`<button type="button" class="btn small dinner-employee-chip ${state.selectedDinnerEmployeeIds.includes(String(e.id))?'primary':''}" data-dinner-employee="${escapeHtml(String(e.id))}" ${e.selectable?'':'disabled title="로그인 계정 연결 필요"'}>${escapeHtml(e.name)} <small>${escapeHtml(e.position||'')}${e.selectable?'':' · 계정연결 필요'}</small></button>`).join("")}</div>`
    :'<span class="muted-text">이 팀에 등록된 직원이 없습니다.</span>';
  box.querySelectorAll('[data-dinner-employee]:not([disabled])').forEach(btn=>btn.addEventListener('click',()=>{
...
async function deleteDinnerRoom(roomId){
  ...
  if(!(await appConfirm(`"${room.title}" 회식방을 삭제할까요? 채팅·참여자·게임 결과도 함께 삭제됩니다.`)))return;
...
async function setupDinnerPresence(roomId){
  ...
  channel.subscribe(async status=>{if(status==='SUBSCRIBED'){await channel.track({user_id:String(state.user.id),name:state.profile?.name||'직원',room_id:String(roomId),online_at:new Date().toISOString()});}});
}
async function leaveDinnerPresence(){if(!state.dinnerPresenceChannel)return;try{await state.dinnerPresenceChannel.untrack();}catch(_){ }try{await supabaseClient.removeChannel(state.dinnerPresenceChannel);}catch(_){ }state.dinnerPresenceChannel=null;state.dinnerOnlineUserIds=new Set();}
...
async function deleteMessengerRoom(){
  ...
  if(!(await appConfirm(`'${roomName}' 방을 폭파하시겠습니까?\n메시지와 참여자 정보가 모두 삭제됩니다.`)))return;
...
function renderDashboard(){
  ...
  renderDashboardLeaveToday();
  renderDashboardSchedule();
}

function renderDashboardLeaveToday(){
  const box=$("dashboardLeaveToday");
  if(!box)return;
  const today=isoDateOffset(0);
  const employeeMap=new Map((state.employees||[]).map(e=>[String(e.id),e]));
  const rows=(state.calendarEntries||[]).filter(x=>
    ["annual","half","quarter"].includes(String(x.entry_type||x.schedule_type||x.type||"")) &&
    String(x.start_date||"")<=today && String(x.end_date||x.start_date||"")>=today
  );
  const groups=[
    ["annual","연차"],
    ["half","반차"],
  ...
  box.innerHTML=groups.map(([type,label])=>{
    const names=rows.filter(x=>String(x.entry_type||x.schedule_type||x.type||"")===type).map(x=>employeeMap.get(String(x.employee_id))?.name||x.employee_name||"직원");
    return `<div class="dashboard-leave-group"><div><b>${label}</b><strong>${names.length}명</strong></div><p>${names.length?names.map(escapeHtml).join(" · "):"없음"}</p></div>`;
  }).join("");
}
...
async function saveContractor(){
  ...
  if(row.meal_count>row.headcount){
    if(!(await appConfirm("식사 인원이 출근 인원보다 많습니다. 그대로 저장할까요?")))return;
  }
...
function canDeleteCorporateCardExpense(){
  return !!(has("card_manage")||state.profile?.is_super_admin);
}
function renderCards(){
  ...
  const canDelete=canDeleteCorporateCardExpense();
  $("cardTable").innerHTML=`<div class="table-wrap"><table><thead><tr>
    <th>사용날짜</th><th>카드소유자</th><th>실사용자</th><th>계정과목</th><th>사용처</th><th>적요/품목</th><th>사용금액</th><th>증빙</th><th>영수증</th><th>비고</th>${canDelete?"<th>관리</th>":""}
  </tr></thead><tbody>${
    rows.length?rows.map(x=>`<tr>
      ...
      ${canDelete?`<td><button class="btn small danger" onclick="deleteCorporateCardExpense('${escapeHtml(String(x.id))}')">삭제</button></td>`:""}
    </tr>`).join(""):`<tr><td colspan="${canDelete?11:10}">등록된 내역이 없습니다.</td></tr>`
  }</tbody></table></div>`;
}
window.deleteCorporateCardExpense=async function(id){
  if(!canDeleteCorporateCardExpense()){toast("관리자만 법인카드 내역을 삭제할 수 있습니다.");return}
  const row=(state.cards||[]).find(x=>String(x.id)===String(id));
  if(!row){toast("삭제할 법인카드 내역을 찾지 못했습니다.");return}
  if(!(await appConfirm(`${row.used_date||""} · ${row.card_owner||""} · ${money(row.amount)} 내역을 삭제할까요?`)))return;
  const {error}=await supabaseClient.from("corporate_card_expenses").delete().eq("id",id);
  if(error){toast("법인카드 내역 삭제 실패: "+error.message);return}
  fastRemoveFromState("cards",id,()=>{renderCards();renderDashboard();});
  if(row.receipt_url)backgroundRefresh(async()=>{const {error}=await supabaseClient.storage.from("receipts").remove([row.receipt_url]);if(error)console.warn("영수증 파일 삭제 실패",error);});
  toast("법인카드 내역을 삭제했습니다.");
}
...
function purchaseActions(x){
  ...
  if(isRecordAdmin())buttons.push(`<button class="btn small danger" onclick="deletePurchaseRequest('${x.id}')">삭제</button>`);
  return buttons.join(" ")||"-";
}
...
async function finalCompletePurchase(id){
  ...
  if(!(await appConfirm(`${x.item_name||"구매 건"}을 검토 단계를 생략하고 바로 구매완료 처리하시겠습니까?`)))return;
...
window.completePurchaseUsage=async function(id){
  ...
  const today=new Date().toISOString().slice(0,10);
  const usageDate=await appPrompt(`'${target.item_name||"구매 물품"}' 사용완료일을 입력하세요.\n형식: YYYY-MM-DD`,{title:"사용완료일",defaultValue:today,placeholder:"YYYY-MM-DD"});
  if(usageDate===null)return;
  if(!/^\d{4}-\d{2}-\d{2}$/.test(usageDate.trim())){toast("사용완료일을 YYYY-MM-DD 형식으로 입력하세요.");return}
  const note=await appPrompt("사용 관련 비고가 있으면 입력하세요. (선택)",{title:"사용 비고",multiline:true,defaultValue:""});
  if(note===null)return;
  if(!(await appConfirm(`사용완료일 ${usageDate.trim()}로 기록할까요?`)))return;
...
window.deleteNotice=async function(id){
  ...
  if(!(await appConfirm(`공지사항 "${notice?.title||""}"을 삭제할까요?`)))return;
...
window.editOrgTeam=async function(id){
  ...
  const name=await appPrompt("팀명을 입력하세요.",{title:"팀명 수정",defaultValue:t.name});if(name===null)return;
  const orderText=await appPrompt("표시순서를 입력하세요.",{title:"표시순서 수정",defaultValue:String(t.sort_order??99),type:"number"});if(orderText===null)return;
...
window.deleteOrgTeam=async function(id){
  const t=state.orgTeams.find(x=>x.id===id);if(!t)return;
  if(!(await appConfirm(`${t.name} 팀을 삭제할까요?`)))return;
...
async function createOrResetEmployeeLeaveYear(mode){
  ...
  if(!(await appConfirm(`${year}년 ${label}하시겠습니까?\n기존 다른 연도 기록은 유지됩니다.`)))return;
...
function renderBusinessTrips(){
  ...
  $("businessTripTable").innerHTML=tableHtml(["구분","출장자","기간","지역","목적","차량","식사","등록자","관리"],rows.map(x=>[
    tripTypeLabel(x.trip_type),businessTripEmployeeNames(x),`${x.start_date} ~ ${x.end_date}`,x.destination||"",x.purpose||"",x.vehicle_number||"미사용",x.exclude_meal!==false?"출장기간 제외":"포함",x.created_by_name||"",isRecordAdmin()?`<button class="btn small danger" onclick="deleteBusinessTrip('${x.id}')">삭제</button>`:"-"
  ]));
}
async function saveBusinessTrip(){
  ...
  if(vehicleId){
    ...
    if(overlap&&!(await appConfirm("선택 차량이 같은 기간에 이미 지정되어 있습니다. 그래도 저장할까요?")))return;
  }
...
window.deleteBusinessTrip=async id=>{if(!(await appConfirm("이 출장을 삭제할까요? 달력의 출장 일정도 함께 삭제됩니다.")))return;await supabaseClient.from("work_calendar_entries").delete().eq("business_trip_id",id);const {error}=await supabaseClient.from("business_trips").delete().eq("id",id);if(error){toast("삭제 실패: "+error.message);return}fastRemoveFromState("businessTrips",id,()=>{renderBusinessTrips();renderCalendar();renderDashboard();});toast("출장을 삭제했습니다.")}
...
window.toggleForkliftManagement=(force)=>{
  ...
  if(!panel)return;
  if(!isAssetAdmin()){toast("관리자만 지게차를 등록·수정할 수 있습니다.");return;}
...
window.openAssetManagement=()=>{
  if(!isAssetAdmin()){toast("관리자만 차량·장비를 등록·수정할 수 있습니다.");return;}
  goPage("vehicles");
  setAssetAdminMode(true);
  document.querySelectorAll("[data-asset-tab]").forEach(b=>b.classList.toggle("primary",b.dataset.assetTab==="manage"));
  $("assetAdminHub")?.scrollIntoView({behavior:"smooth",block:"start"});
};
window.exitAssetManagement=()=>{
  setAssetAdminMode(false);
  state.vehicleViewGroup=state.vehicleViewGroup||"company";
  activateVehicleTab("trip");
  renderVehicles();
};
window.openVehicleAdminSection=(group)=>{
  ...
  renderVehicles();
  $("vehicleBasicPanel")?.scrollIntoView({behavior:"smooth",block:"start"});
};
window.openForkliftAdminFromHub=()=>{
  if(!isAssetAdmin()){toast("관리자만 사용할 수 있습니다.");return;}
  goPage("forklift");
  const btn=$("forkliftManageToggleBtn");if(btn)btn.style.display="";
  toggleForkliftManagement(true);
};
...
function renderCompanyEvents(){if(!$("companyEventTable"))return;const month=$("eventMonthFilter").value||isoDateOffset(0).slice(0,7);const rows=state.companyEvents.filter(x=>String(x.start_date).startsWith(month));$("companyEventTable").innerHTML=tableHtml(["상태","구분","행사명","기간","시간","담당","인원","장소","관리"],rows.map(x=>[eventStatusBadge(x),eventTypeLabel(x.event_type),x.title,`${x.start_date}${x.end_date!==x.start_date?` ~ ${x.end_date}`:""}`,x.event_time||"",x.manager_name||"",`${x.participant_count||0}명`,x.location||"",isRecordAdmin()?`<button class="btn small danger" onclick="deleteCompanyEvent('${x.id}')">삭제</button>`:"-"]));const today=isoDateOffset(0);$("eventUpcomingList").innerHTML=state.companyEvents.filter(x=>x.end_date>=today).slice(0,8).map(x=>`<div class="list-item"><b>${eventStatusBadge(x)} ${eventTypeLabel(x.event_type)} · ${escapeHtml(x.title)}</b><small>${x.start_date} ${escapeHtml(x.event_time||"")} · ${escapeHtml(x.location||"")}</small></div>`).join("")||`<div class="empty">예정된 일정이 없습니다.</div>`}
...
window.deleteMeeting=async id=>{if(!(await appConfirm("예약을 취소할까요?")))return;const {error}=await supabaseClient.from("meeting_room_bookings").delete().eq("id",id);if(error){toast("취소 실패: "+error.message);return}fastRemoveFromState("meetingBookings",id,()=>{renderMeetings();renderDashboard();});toast("예약을 취소했습니다.")}
...
window.deleteVehicleTrip=async id=>{if(!(await appConfirm("이 운행일지를 삭제할까요?")))return;const {error}=await supabaseClient.from("vehicle_trip_logs").delete().eq("id",id);if(error){toast("삭제 실패: "+error.message);return}fastRemoveFromState("vehicleTrips",id,renderVehicles);toast("운행일지를 삭제했습니다.")}
...
window.deleteVehicleMaintenance=async id=>{if(!(await appConfirm("이 정비 내역을 삭제할까요?")))return;const {error}=await supabaseClient.from("vehicle_maintenance").delete().eq("id",id);if(error){toast("삭제 실패: "+error.message);return}fastRemoveFromState("vehicleMaintenance",id,renderVehicles);toast("정비 내역을 삭제했습니다.")}
function vehicleWorkflowLabel(v){return ({reported:"금액확인 대기",amount_checked:"최종결재 대기",approved:"결재완료",rejected:"반려"})[v]||v||"보고"}
function vehicleMaintenanceActions(x){const a=[];if(currentUserIs("김헌정")&&(x.workflow_status||"reported")==="reported")a.push(`<button class="btn small primary" onclick="checkMaintenanceAmount('vehicle','${x.id}')">금액 확인</button>`);if((currentUserIs("손동오")||state.profile?.is_super_admin)&&x.workflow_status==="amount_checked"){a.push(`<button class="btn small primary" onclick="approveMaintenance('vehicle','${x.id}',true)">결재</button>`);a.push(`<button class="btn small danger" onclick="approveMaintenance('vehicle','${x.id}',false)">반려</button>`)}if(x.reported_by===state.user.id||isRecordAdmin())a.push(`<button class="btn small" onclick="editVehicleMaintenance('${x.id}')">수정</button>`);if(isRecordAdmin())a.push(`<button class="btn small danger" onclick="deleteVehicleMaintenance('${x.id}')">삭제</button>`);return a.join(" ")||"-"}
async function checkMaintenanceAmount(kind,id){const table=kind==="forklift"?"forklift_maintenance":"vehicle_maintenance";const list=kind==="forklift"?state.forkliftMaintenance:state.vehicleMaintenance;const x=list.find(r=>String(r.id)===String(id));const amount=await appPrompt("확인할 금액을 입력하세요.",{title:"정비 금액 확인",defaultValue:String(x?.cost||0),type:"number"});if(amount===null)return;const value=Number(String(amount).replace(/,/g,""));if(!Number.isFinite(value)||value<0){toast("금액을 확인하세요.");return}const {error}=await supabaseClient.from(table).update({cost:value,workflow_status:"amount_checked",amount_checked_by:state.user.id,amount_checked_by_name:state.profile?.name||"김헌정",amount_checked_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",id);if(error){toast("금액 확인 실패: "+error.message);return}await (kind==="forklift"?loadForkliftMaintenance():loadVehicleMaintenance());kind==="forklift"?renderForklifts():renderVehicles();renderPendingWorkAlerts(true);toast("금액 확인을 완료했습니다. 손동오 관리자에게 최종 결재 알림이 전달됩니다.")}
window.checkMaintenanceAmount=checkMaintenanceAmount;
async function approveMaintenance(kind,id,approved){const table=kind==="forklift"?"forklift_maintenance":"vehicle_maintenance";if(!(await appConfirm(approved?"최종 결재하시겠습니까?":"반려하시겠습니까?")))return;const {error}=await supabaseClient.from(table).update({workflow_status:approved?"approved":"rejected",approved_by:state.user.id,approved_by_name:state.profile?.name||"손동오",approved_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",id);if(error){toast("처리 실패: "+error.message);return}await (kind==="forklift"?loadForkliftMaintenance():loadVehicleMaintenance());kind==="forklift"?renderForklifts():renderVehicles();renderPendingWorkAlerts();toast(approved?"최종 결재했습니다.":"반려했습니다.")}
...
function forkliftMaintenanceActions(x){const a=[];if(currentUserIs("김헌정")&&(x.workflow_status||"reported")==="reported")a.push(`<button class="btn small primary" onclick="checkMaintenanceAmount('forklift','${x.id}')">금액 확인</button>`);if((currentUserIs("손동오")||state.profile?.is_super_admin)&&x.workflow_status==="amount_checked"){a.push(`<button class="btn small primary" onclick="approveMaintenance('forklift','${x.id}',true)">결재</button>`);a.push(`<button class="btn small danger" onclick="approveMaintenance('forklift','${x.id}',false)">반려</button>`)}if(x.reported_by===state.user.id||isRecordAdmin())a.push(`<button class="btn small" onclick="editForkliftMaintenance('${x.id}')">수정</button>`);if(isRecordAdmin())a.push(`<button class="btn small danger" onclick="deleteForkliftMaintenance('${x.id}')">삭제</button>`);return a.join(" ")||"-"}
window.deleteForklift=async function(id){if(!has("vehicles_manage")){toast("관리자만 삭제할 수 있습니다.");return}if(!(await appConfirm("이 지게차를 삭제할까요?")))return;const {error}=await supabaseClient.from("forklift_assets").delete().eq("id",id);if(error){toast("삭제 실패: "+error.message);return}fastRemoveFromState("forkliftAssets",id,renderForklifts);toast("지게차를 삭제했습니다.")}
function forkliftCard(x){const electric=(x.equipment_type||"전동")==="전동";return `<article class="vehicle-card"><div class="vehicle-card-head"><b>${escapeHtml(x.asset_name)}</b><span class="badge">${escapeHtml(x.equipment_type||"전동")}</span></div><small>담당 ${escapeHtml(x.manager_name||"-")}</small><div class="vehicle-card-metrics"><span>가동 ${Number(x.current_hours||0).toLocaleString()}시간</span><span>${electric?`증류수 ${escapeHtml(x.last_distilled_water_date||"미등록")}`:"디젤 정비대상"}</span></div><div class="inline-actions">${isAssetAdmin()?`<button class="btn small" onclick="editForklift('${x.id}')">수정</button><button class="btn small danger" onclick="deleteForklift('${x.id}')">삭제</button>`:""}</div></article>`}

window.deleteForkliftMaintenance=async function(id){if(!isRecordAdmin()){toast("관리자만 삭제할 수 있습니다.");return}if(!(await appConfirm("이 지게차 정비 이력을 삭제할까요?")))return;const {error}=await supabaseClient.from("forklift_maintenance").delete().eq("id",id);if(error){toast("삭제 실패: "+error.message);return}fastRemoveFromState("forkliftMaintenance",id,renderForklifts);toast("지게차 정비 이력을 삭제했습니다.")};
...
async function deleteSelectedTestData(){
  ...
  if(!(await appConfirm(`${label} ${testDataSelection.selected.size}건을 삭제할까요?`)))return;
  ...
  testDataSelection.rows=testDataSelection.rows.filter(r=>!deleted.has(String(r.id)));
  testDataSelection.selected.clear();
  renderTestDataRecords();updateTestDataSelectedCount();
  toast(`${label} ${ids.length}건을 삭제했습니다.`);
}
