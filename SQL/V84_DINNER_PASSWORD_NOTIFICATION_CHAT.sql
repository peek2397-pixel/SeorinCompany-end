...
create table if not exists public.dinner_rooms (
 ...
 password_hash text, messenger_room_id uuid, created_by uuid not null, created_by_name text, created_at timestamptz not null default now()
);
alter table public.dinner_rooms add column if not exists password_hash text;
...
create table if not exists public.dinner_room_messages (
 id uuid primary key default gen_random_uuid(),room_id uuid not null references public.dinner_rooms(id) on delete cascade,
 sender_id uuid not null,sender_name text not null,message_text text not null check(length(trim(message_text)) between 1 and 3000),created_at timestamptz not null default now()
);
create index if not exists idx_dinner_room_messages_room_time on public.dinner_room_messages(room_id,created_at);

create or replace function public.create_dinner_room_secure(p_title text,p_dinner_date date,p_password text,p_members jsonb)
returns uuid language plpgsql security definer set search_path=public as $$
declare v_room uuid; v_member jsonb; v_name text;
begin
 ...
 insert into public.dinner_rooms(title,dinner_date,password_hash,created_by,created_by_name)
 values(trim(p_title),coalesce(p_dinner_date,current_date),crypt(p_password,gen_salt('bf')),auth.uid(),coalesce(v_name,'관리자')) returning id into v_room;
 for v_member in select * from jsonb_array_elements(coalesce(p_members,'[]'::jsonb)) loop
   ...
   values(v_room,(v_member->>'user_id')::uuid,coalesce(nullif(trim(v_member->>'member_name'),''),'직원')) on conflict(room_id,user_id) do nothing;
 end loop;
...
create or replace function public.verify_dinner_room_password(p_room_id uuid,p_password text)
